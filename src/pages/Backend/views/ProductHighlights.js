...
import ModalsEdit from "../../../component/highlightModalsEdit.js"
import ModalsAdd from "../../../component/highlightModalsAdd.js"
...
const ProductHighlights = () => {
    ...
    const getData = (id, title_id, title_en, description_id, description_en, image) => {
        let tempData = [id, title_id, title_en, description_id, description_en, image]
    ...
    const handleDelete = (itemId) => {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    let URL = `https://staging.javaagroglobalindo.com/highlights/delete/${itemId}`
                    const fetchDelete = axios.delete(URL, {
                        withCredentials: false,
                        headers: {
                            Authorization: token
                        },
                                            ...
                                            {data && (data.map((item, i) => (
                                                <tr key={i}>
                                                    ...
                                                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 flex justify-between">
                                                        <button className="text-white bg-green-500 px-6 py-2 rounded-full hover:bg-green-400" onClick={() => getData(item.id, item.title_id, item.title_en, item.description_id, item.description_en, item.image)}>Edit</button>
                                                    ...
                                                    <ModalsEdit open={modalEdit} onClose={() => setModalEdit(false)} id={tempData[1]} title_id={tempData[2]} title_en={tempData[3]} description_id={tempData[4]} description_en={tempData[5]} image={tempData[6]} >
                                                        {item.id}
                                                    </ModalsEdit>
                                                </tr>
                                            )))}
